...
export const LogService = {
  ...
  async deleteLogsForMonth(month: number, year: number): Promise<void> {
    const allLogs = await this.getAllDailyLogs();
    const updatedLogs: { [date: string]: DailyLog } = {};

    for (const date in allLogs) {
      const logDate = new Date(date);
      // Keep logs that DO NOT match the specified month and year
      if (logDate.getMonth() !== month || logDate.getFullYear() !== year) {
        updatedLogs[date] = allLogs[date];
      }
    }

    await AsyncStorage.setItem(STORAGE_KEYS.DAILY_LOGS, JSON.stringify(updatedLogs));
    console.log(`[LogService] Deleted all logs for month ${month + 1}/${year}.`);
  },
